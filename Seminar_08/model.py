...
def get_list():
    with open('catalog.csv', encoding="utf-8") as r_file:
        reader = csv.reader(r_file, delimiter=';')
        return list(reader)


def add_employee_to_list(employee):
    with open('catalog.csv', 'a', encoding="utf-8", newline='') as a_file:
...
def update_employee(number, string):
    try:
        with open('catalog.csv', 'r', encoding="utf-8", newline='') as r_file:
            reader = csv.reader(r_file, delimiter=';')
            data = list(reader)
            data[number] = string
        with open('catalog.csv', 'w', encoding="utf-8", newline='') as w_file:
    ...
    with open('catalog.csv', 'r', encoding="utf-8", newline='') as r_file:
        ...
        for i, sotrudnik in enumerate(data):
            if i in numbers:
                ...
                exp_i.append(i)
                numbers.remove(i)
    with open('out.csv', 'w', encoding="utf-8", newline='') as w_file:
        writer = csv.writer(w_file, delimiter=';')
...
def import_exployees_file():
    with open('impr.csv', 'r', encoding="utf-8", newline='') as r_file:
